  ...
  private buildEmbed(): EmbedBuilder {
    return new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: '🛡️ 自動管理與安全防護 (AutoMod)',
          value:
            '• `/automod 新增 [禁用詞]`：設定伺服器禁用詞（限管理員）。\n' +
            '• `/automod 移除 [禁用詞]`：解除禁用詞（限管理員）。\n' +
            '• `/automod 列表`：查看目前所有禁用詞（限管理員）。\n' +
            '• 🚫 **禁言處置**：一般成員若打出禁用詞，訊息會被自動刪除並被**禁言 60 秒**！\n' +
            '• 🎣 **防詐騙機制**：自動偵測含 `@everyone` 的釣魚連結，若跨頻道發送會自動 **Ban** 該成員並清理訊息！'
        },
        {
          name: '🧠 AI 長期記憶系統',
          value:
            '• `/記憶 查看` / `/我的記憶`：查看波波對你記錄的長期記憶。\n' +
            '• `/記憶 開啟` / `/記憶 關閉`：控制波波是否記錄你的個人特徵。\n' +
            '• `/記憶 設定 [內容]`：手動寫入/覆蓋你的長期記憶。\n' +
            '• `/記憶 清除`：清空你的長期記憶。\n' +
            '• `/記憶 狀態`：管理員查看 Mem0 背景寫入 queue、節流與錯誤計數。\n' +
