...
export function isPotentialStockQuery(prompt: string): boolean {
  ...
  if (
    ...
    (taiwanStockMap[normalized] || COMMON_STOCK_MAP[normalized])
  ) {
    return true
  }
  ...
  const hasStockTarget =
    ...
    Object.keys(taiwanStockMap).some(
      key => key.length >= 2 && normalized.includes(key.toLowerCase())
    ) ||
    Object.keys(NICKNAME_MAP).some(
      key => key.length >= 2 && normalized.includes(key.toLowerCase())
    ) ||
    Object.keys(COMMON_STOCK_MAP).some(
      key => key.length >= 2 && normalized.includes(key.toLowerCase())
    )
 ...
 */
export const getChineseNameWithAI = async (
  ticker: string,
  englishName?: string
): Promise<string | null> => {
  const apiKey = getApiKey()
    ...
    const response = await executeGenAI(ai =>
      ai.models.generateContent({
        ...
        contents: [
          {
            text:
              `你是一個公司名稱翻譯助手。請將以下上市公司的英文名稱或代碼，翻譯成其在台灣或全球股市最常見的繁體中文公司簡稱。\n` +
              `規範與範例：\n` +
