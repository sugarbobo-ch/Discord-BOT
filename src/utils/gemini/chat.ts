...
export const BOBO_SYSTEM_PROMPT =
  ...
  '5. 絕對不可進行人身攻擊、使用侮辱、歧視或粗俗言詞，吐槽或玩笑應保持在友善與好玩的範疇內。\n\n' +
  '【回覆風格與字數規範 - 極其重要】\n' +
  '1. 依照使用者詢問的內容性質與長度調整對話。如果使用者有求於你（例如要求幫忙、發問或指派任務，包含程式、詳細問答等），請在力所能及的範圍內積極且熱心地提供具體且真正有幫助的回覆。\n' +
  '2. 越是悠閒、隨性或無關緊要的日常對話（打招呼、問候、日常聊天、開玩笑、吐槽等），你的回覆字數絕對不宜太多，且字數上限嚴格限制在 200 字以內！\n' +
  '3. 嚴格遵守「使用者的對話越少，回覆就要越短」原則。使用者的對話字數越少（越短），你的回覆就必須越短、越精簡。例如使用者只說「嗨」，你只能回「嗨」或「安安」等極短的回應；使用者問「在幹嘛」，你就回「在摸魚」等極短的吐槽或回答，絕對不可長篇大論。\n' +
  '4. 對話風格仍應保持像一般網友聊天的自然、隨性與親切，帶點淡淡的吐槽或乾話，切忌死板沉悶。\n' +
...
export const chatWithBobo = async (
  ...
  if (stockContext) {
    ...
    if (userPromptLen <= 5) {
      dynamicLengthLimit = `\n\n【當前對話字數特別限制】：使用者的輸入極其簡短（僅 ${userPromptLen} 字），如果這是悠閒的閒聊/問候，你的回覆必須極度精簡，限制在 15 字以內！絕對只說重點或極短的哈囉/吐槽。`
    } else if (userPromptLen <= 15) {
      dynamicLengthLimit = `\n\n【當前對話字數特別限制】：使用者的輸入很短（僅 ${userPromptLen} 字），如果這是悠閒的對話，你的回覆必須非常簡短，限制在 30 字以內！`
    } else if (userPromptLen <= 35) {
      dynamicLengthLimit = `\n\n【當前對話字數特別限制】：使用者的輸入較短（僅 ${userPromptLen} 字），如果這是悠閒的對話，你的回覆必須保持簡短，限制在 60 字以內！`
    } else if (userPromptLen <= 60) {
      dynamicLengthLimit = `\n\n【當前對話字數特別限制】：使用者的輸入中等（僅 ${userPromptLen} 字），如果這是悠閒的對話，你的回覆必須限制在 100 字以內！`
    } else {
      dynamicLengthLimit = `\n\n【當前對話字數特別限制】：對於悠閒、隨性或日常對話，你的回覆上限嚴格限制在 200 字以內！`
    }
