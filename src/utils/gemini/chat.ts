...
export const ANALYST_SYSTEM_PROMPT =
  ...
  '【長度限制 - 極其重要】\n' +
  'Discord 單則訊息有 2000 字元硬性上限。請將你的總分析報告精準濃縮在 500~800 字以內（約 1500 字元以下），精準列出關鍵重點，絕對不可產生冗長流水帳或超過 1800 字元！\n\n' +
  '【格式規範 - 極其重要】\n' +
...
export const BOBO_SYSTEM_PROMPT =
  ...
  '【回覆風格與字數規範 - 極其重要】\n' +
  '1. 依照使用者詢問的內容性質與長度調整對話。如果使用者有求於你（例如要求幫忙、發問或指派任務，包含程式、詳細問答等），請在力所能及的範圍內積極且熱心地提供具體且真正有幫助的回覆。但請注意：Discord 單則訊息有 2000 字元上限，無論回答多複雜，請將整體內容精確濃縮在 800 字以內（約 1500 字元以下），嚴格禁止產生冗長流水帳！\n' +
...
export const chatWithBobo = async (
  ...
  try {
    ...
    // Discord 2000 字元長度保護：確保整體回覆包含簽章始終小於 2000 字元，避免觸發多段分拆
    if (replyText.length > 1900) {
      const truncated = replyText.substring(0, 1850)
      ...
      replyText =
        (lastBoundary > 1200 ? truncated.substring(0, lastBoundary + 1) : truncated).trim() +
        '\n\n...(字數已達單則上限，已自動為您摘要核心重點)'
    }
